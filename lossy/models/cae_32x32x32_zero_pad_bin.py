...
class CAE(nn.Module):
    ...
    def forward(self, x):
        ...
        eblock4 = self.e_block_4(eblock3) + eblock3
        ec4 = self.e_conv_4(eblock4)  # in [-1, 1] from tanh activation

        # stochastic binarization
        with torch.no_grad():
            rand = torch.rand(ec4.shape).cuda()
            prob = (1 + ec4) / 2
            eps = torch.zeros(ec4.shape).cuda()
            eps[rand <= prob] = (1 - ec4)[rand <= prob]
            eps[rand > prob] = (-ec4 - 1)[rand > prob]

        # encoded tensor
        self.encoded = 0.5 * (ec4 + eps + 1)  # (-1|1) -> (0|1)
    ...
    def decode(self, encoded):
        ...
        dblock4 = self.d_block_4(dblock3) + dblock3
        uc2 = self.d_up_conv_2(dblock3)
        uc3 = self.d_up_conv_3(dblock4)
        dec = self.d_up_conv_4(uc3)
